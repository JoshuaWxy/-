...
/*    qDebug()<<"时间基数:"<<m_codecContext->framerate.num;   */
               // 像素格式（这里通过编码器赋值，不需要自己指定）
     m_codecContext->time_base = {1, 22};                   //设置时间基，20为分母，1为分子，表示以1/20秒时间间隔播放一帧图像
    m_codecContext->framerate = {22, 1};
    m_codecContext->bit_rate = m_recording_qulity;
    qDebug()<<"质量:"<<m_recording_qulity;                                  // 目标的码率，即采样的码率；显然，采样码率越大，视频大小越大，画质越高
...
void VideoCodec::setRecording_qulity(int size)
{
    m_recording_qulity=size;
}
