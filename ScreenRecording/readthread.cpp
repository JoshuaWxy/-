...
ReadThread::ReadThread(QObject *parent) : QThread(parent)
{
    ...
    qRegisterMetaType<PlayState>("PlayState");    // 注册自定义枚举类型，否则信号槽无法发
...
void ReadThread::setPath(const QString &path)
{
    ...
    {
        QMessageBox::warning(nullptr, "注意~", "输入文件没有后缀名");
        m_path.clear();
        return;
    }
    m_path = path;
...
void ReadThread::run()
{
    ...
    if(ret)
    {
        ...
        qDebug()<<"平均帧率:"<<m_videoDecode->avgFrameRate() .x()<<" "<<m_videoDecode->avgFrameRate().y();
        if(!m_play)
        {
            qDebug() << "打开输出文件失败！";
        }
...
void ReadThread::do_recordingSetting(QString rate, QString size, QString qulity)
{
    m_videoDecode->setRecording_frame_rate(rate);
    m_videoDecode->setRecording_screen_size(size);
    if(qulity=="中等质量")
    m_videoCodec->setRecording_qulity(4000000);
    else if(qulity=="高质量")
    m_videoCodec->setRecording_qulity(8000000);
    else if(qulity=="低质量")
    m_videoCodec->setRecording_qulity(1000000);
    qDebug()<<rate<<"!";
    qDebug()<<size<<"!";
    qDebug()<<qulity<<"!";
}
